...
if not os.getenv("DASHSCOPE_API_KEY"):
    raise RuntimeError("未配置 DASHSCOPE_API_KEY，请在 .env 文件中设置")
...
# ==================== 模型 ====================
model = ChatTongyi(model=MODEL_NAME, temperature=TEMPERATURE, streaming=True)
...
def _should_skip_content(content: str) -> bool:
    ...
    if stripped.startswith(SKIP_PREFIXES):
        return True
    if any(x in stripped for x in SKIP_CONTAINS):
        return True
    return False
...
def get_product_review_streaming(
    user_query: str, thread_id: str = "default_session"
...
5. 在报告末尾，用 Markdown 表格格式给出推荐的几款产品的对比评分。

请严格按照以上要求直接输出最终报告。"""

        config = {"configurable": {"thread_id": thread_id}}

        # 3. 流式调用（带缓冲区，避免chunk截断导致过滤失效）
        buffer = ""
        for chunk in agent.stream(
            ...
            # 缓冲区积累足够或遇到换行时输出，避免关键词被截断
            if len(buffer) >= 20 or "\n" in buffer:
                if not _should_skip_content(buffer):
                    full_content += buffer
                    yield buffer.replace("~", "\\~")
                buffer = ""

        # 输出剩余缓冲
        if buffer and not _should_skip_content(buffer):
            full_content += buffer
            yield buffer.replace("~", "\\~")
        ...
        if full_content.strip():
            rag_mem.add_report(
                full_content,
                metadata={"query": user_query, "thread_id": thread_id},
            )
